...
@Controller
@RequestMapping("/AlisendNoticeController")
public class AliSendNoticeController extends BaseController{
	
	private static Logger logger = Logger.getLogger(AliSendNoticeController.class);
	...
	ConstPool constPool;
	@Autowired
	private SendPhoneNoticeController sendPhoneNoticeController;
	
	/**	 * 
	 * @api {post} /AlisendNoticeController/AliSendNotice 群发通知(首页-扫码)new
	 * @apiPermission user
	 * @apiDescription  群发送至 jiong.zheng
	 * @apiparam {String} accountId 钱包ID
	 * @apiparam {String} aliPhoneAndNum 运单号和编号和netId, 运单号-编号-netId-网络名称|运单号-编号-netId-网络名称 (阿里的)
	 * @apiparam {String} phoneAndNum 手机号和编号 ，电话号-编号-是否关注微信-运单号-网络名称-姓名|电话号-编号-是否关注微信-运单号-网络名称-姓名  这样  0.关注 1.未关注 (普通的)[如果姓名没有传空,但是 - 要拼上]
	 * @apiparam {String} noticePhone 通知号码
	 * @apiparam {Long} memberId 当前收派员的memberId
	 * @apiparam {String} content 短信内容
	 * @apiparam {String} flag 1：仅短信  5:微信优先
	 ...
	 * @apiGroup 通知记录
	 * @apiSampleRequest /AlisendNoticeController/AliSendNotice
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/AliSendNotice",method={RequestMethod.POST,RequestMethod.GET})
	public String AliSendNotice(String accountId,String aliPhoneAndNum,String noticePhone,Long memberId,String content,Short flag, String pickupTime, 
			String pickupAddr, String compName, String phoneAndNum, String templateId, String timeStamp, String versionCode){
		logger.info("阿里-普通短信通知accountId:"+accountId+",phoneAndNum:"+phoneAndNum+",noticePhone:"+
				...
				",pickupAddr:"+pickupAddr+",compName: "+compName+" ,aliPhoneAndNum: "+aliPhoneAndNum+" ,templateId: "+templateId+ " ,timeStamp: "+timeStamp+" ,versionCode: "+versionCode);
		...
		//防止重复提交1, 如果阿里大于不是空进来验证重复提交的
		if(!PubMethod.isEmpty(aliPhoneAndNum)){
			if(!PubMethod.isEmpty(timeStamp) && !PubMethod.isEmpty(versionCode)){
				logger.info("**********timeStamp: "+timeStamp);
				String obj = taskMassNoticeService.queryOnlyRepeatTag(memberId,timeStamp, accountId, noticePhone, content, versionCode, "");
		...
		//判断余额是否足够
		Map<String,Object> resultMap = this.isSmsBalance(memberId, accountId, content, (short)9, noticePhone, md5Hex, aliPhoneAndNum, phoneAndNum);
		...
		if(!PubMethod.isEmpty(phoneAndNum)){
			...
			String result = sendPhoneNoticeController.sendPhoneNotice(accountId, phoneAndNum, noticePhone, memberId, content, flag, "", pickupTime, pickupAddr, compName, templateId);
			...
			String returnjson = sendPhoneNoticemsService.queryMsgTemplate(accountId, content, phoneAndNum, memberId,null,null,false,true,
		...
		if(PubMethod.isEmpty(flag)){
			...
			for(int j=0;j<arrs.length;j++){
				...
				if(i==0){
					phoneAndWaybillNum+=waybill+"-"+num+"-"+arrs[j].split("-")[2]+"-"+arrs[j].split("-")[3]+"-"+resultMap.get(waybill); //运单号-编号-netId-网络名称-是否扣费
					i++;
				}else{
					phoneAndWaybillNum+="|"+waybill+"-"+num+"-"+arrs[j].split("-")[2]+"-"+arrs[j].split("-")[3]+"-"+resultMap.get(waybill); 
				}
			}
			
			String aliResult = alisendNoticeService.sendAliSms(accountId,content, phoneAndWaybillNum, memberId, null, null,false,true,null,null,(short)1, 
					pickupTime,pickupAddr,compName,flag, noticePhone, md5Hex, versionCode);
			result = aliResult;
	...
	public Map<String,Object> isSmsBalance(Long memberId,String accountId, String content, Short flag, String noticePhone, String md5Hex, 
			String aliPhoneAndNum, String phoneAndNum){
		logger.info("进来验证余额....");
