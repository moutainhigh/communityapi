...
public class AppUpgradeController extends BaseController{
	...
	public String queryAppUpgrade(Short appType,String selfVersion,Short phoneType){
		if(PubMethod.isEmpty(appType)){
			return paramsFailure("net.okdi.api.controller.AppUpgradeController.queryAppUpgrade.001", "appType不能为空！");
		}
		if(PubMethod.isEmpty(selfVersion)){
			return paramsFailure("net.okdi.api.controller.AppUpgradeController.queryAppUpgrade.002", "selfVersion不能为空！");
		}
		if(PubMethod.isEmpty(phoneType)){
			return paramsFailure("net.okdi.api.controller.AppUpgradeController.queryAppUpgrade.003", "phoneType不能为空！");
		}
		try {
			return this.appUpgradeService.queryAppUpgrade(appType, selfVersion, phoneType);
		} catch (Exception e) {
			e.printStackTrace();
