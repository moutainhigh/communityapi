...
import net.okdi.apiV4.dao.ExpCustomerInfoNewMapper;
...
@Service
public class ExpCustomerInfoNewServiceImpl extends AbstractHttpClient implements ExpCustomerInfoNewService {

	@Autowired
	ExpCustomerInfoNewMapper expCustomerInfoNewMapper;
	@Autowired
	 ...
	 */
	@Override
	public Map<String, Object> queryCustomers(Long memberId, String customerName, Short customerType, String customerPhone,Long expMemberId,
			Page page) {
		...
		if (ehcacheService.getByKey("customerInfoCache", cacheParam)) { //缓存不为空
			...
			page.setTotal(count);
			addCache(memberId, cacheCount, count);
			addCache(memberId, cacheParam, listCustomersId);
		}

		for (int i = 0; i < listCustomersId.size(); i++) {
			ExpCustomerInfo eci = getByCustomerId(memberId, Long.valueOf(listCustomersId.get(i).toString()));
	...
	@Override
	public List<ExpCustomerInfo> queryCustomersJDW(Long memberId, String quertCondition) {
		Page page = new Page();
...
//		String customerPhone = quertCondition;
	...
	private List<ExpCustomerInfo> selectContactNameByMemberId(Long memberId,
		...
		if (ehcacheService.getByKey("customerInfoCache", cacheParam)) {
			...
			lsExpInfo = expCustomerInfoNewMapper.selectContactNameByMemberId(m);
	...
	public List<ExpCustomerInfo> selectByContactName(Long compId, String customerName){
		...
		if (ehcacheService.getByKey("customerInfoCache", cacheParam)) {
			...
			lsExpInfo = expCustomerInfoNewMapper.selectByContactName(m);
	...
	public List<ExpCustomerInfo> queryByErpIdandCompId(Long compId,Long erpId){
		...
		if (ehcacheService.getByKey("customerInfoCache", cacheParam)) { //缓存不为空
			...
			map.put("erpId", erpId);
			listExp = expCustomerInfoNewMapper.queryByErpIdandCompId(map);
	...
	public ExpCustomerInfo getByCustomerId(Long memberId, Long customerId) {
		...
		if (ehcacheService.getByKey("customerInfoCache", cacheParam)) { //缓存不为空
			...
			eci = expCustomerInfoNewMapper.selectByKey(customerId);
...
//			addCache(compId, cacheParam, eci);
	...
	public void clearDiscountGroupIdByCompId(Long compId,Long discountGroupId){
		...
		expCustomerInfoNewMapper.clearDiscountGroupIdByCompId(map);
	...
	public String insertCustomer(Long compId, String customerName, Short customerType, String customerPhone,Long erpCustomerId,
		...
		expCustomerInfoNewMapper.insert(eci);
	...
	public String updateCustomer(Long customerId, Long compId, String customerName, Short customerType,
		...
		eci.setAgencyFee(agencyFee);
		eci.setSettleType(settleType);
		expCustomerInfoNewMapper.updateByPrimaryKey(eci);
	...
	public String deleteCustomer(Long customerId, Long compId) {
		expCustomerInfoNewMapper.deleteByPrimaryKey(customerId);
	...
	public String addContacts(Long compId, Long customerId, String contactsName, String contactsPhone,Long memberId) {
		...
		ehcacheService.remove("customerInfoCache", memberId + "Contacts");
...
//			}
	...
	public String insertCustomerV4(Long compId, String customerName,Byte gender,
		...
		expCustomerInfoNewMapper.insertV4(eci);
...
//		}
	...
	public List<ExpCustomerInfo> selectCustomer(Long compId,Long memberId) {
		List<ExpCustomerInfo> list=expCustomerInfoNewMapper.selectCustomer(memberId);
		return list;
	}
	...
	public String editCustomer(Long customerId, Long compId,
		...
		eci.setCompAddress(compName);
		eci.setMemberId(memberId);
		expCustomerInfoNewMapper.editCustomer(eci);
		removeCompIdCache(compId);
		removeCompIdCache(memberId);
		return String.valueOf(customerId);
	}
	...
	public String addCustomerType(String addCustomerIds,String delCustomerIds,String customerType,Long compId,Long memberId) {
		...
		for(String customerId:str){
			...
			expCustomerInfoNewMapper.addCustomerType(map);
		}
		//删除标签
		String[] strd=delCustomerIds.split(",");
		for(String customerId:strd){
			map.put("customerId", customerId);
			expCustomerInfoNewMapper.deleteCustomerType(map);
		}
		removeCompIdCache(compId);
		removeCompIdCache(memberId);
		return "1";
	...
	public String addContactsInfo(Long compId, String mobileData,Long memberId) {
		...
		for (Object object : jsonArray) {
			...
			List<ExpCustomerInfo> list=expCustomerInfoNewMapper.selectCustomersByMobile(map1);
			if(list.size()>0){
				continue;
			}
			Long customerId = IdWorker.getIdWorker().nextId();
...
//		Map<String, Object> mapList=parTaskInfoMapperV4.queryTakeTaskDetailByTaskId(Long.parseLong(tradeNum));
		...
		System.out.println("=======走的取件新方法    saomaCreate   server＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝");
...
//		    String success=String.valueOf(JSON.parseObject(cancelResult).get("success"));
		...
		//查询当前人的名字
		String memberName = memberInfoMapperV4.findMemberNameByMemberId(actorMemberId);
