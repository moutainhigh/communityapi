...
public class CollectionNewServiceImpl implements CollectionNewService {
	...
	public static final Log logger = LogFactory.getLog(CollectionNewServiceImpl.class);
	...
	//	map = this.redisService.get("getMemberInfoCollection", memberId, Map.class);
		...
		//}
			System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ---- getMemberInfo "+ map.get("memberId"));
		return map;
	}
	...
	public Map<String,Object> isHasCollection(String compName, String compAddress) {
		...
		System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa 验证代收点存在的 ---- isHasCollection "+ map.get("type"));
		return map;
	}
	//店员创建不存在的代收点
	@Override
	public String insertClerkAudit1(String compName, String compAddress,
		...
		map.put("agentType", agentType);// 代收点类型
		try {
		...
		try {
			compInfoMapper.insertBasCompinfoNew(map);
...
//			basCompAuditMapper.insertbasCompAudit(map);
			...
			//---2016年4月27日13:45:25--by hu.zhao...解决更换快递代收业务审核信息问题
			doGsInfo(memberId, roleId, id.toString());
			//---2016年4月27日13:45:25--by hu.zhao...解决更换快递代收业务审核信息问题
			...
			if (PubMethod.isEmpty(auditId)) {
				...
				this.collectionMapper.updateIdentity(memberName, memberId);
			...
			// 更新member_info 表，将身份证号更新进去
			this.collectionMapper.updateIdentity(null, memberName, memberId);
		} catch (Exception e) {
		...
		System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa 店员创建不存在的代收代点的 ---- insertClerkAudit1 :"+ id.toString());
		...
		this.redisService.removeAll("findIdentity");
...
//		basCompAuditMapper.insertbasCompAudit(map);
		...
		//---2016年4月27日13:45:25--by hu.zhao...解决更换快递代收业务审核信息问题
		doGsInfo(memberId, roleId, compId);
		//---2016年4月27日13:45:25--by hu.zhao...解决更换快递代收业务审核信息问题
	...
	public String insertShopkeeperAudit1(String compName, String compAddress,
		...
		// TODO Auto-generated method stub
		this.deleteKdInfo(memberId);//删除快递认证
		System.out.println("insertShopkeeperAudit1 -----------------  开始 ");
		long start = System.currentTimeMillis();
		/*Map<String, Object> Resultmap = parseService.parseAddrByList(latitude,
				longitude);
		System.out.println("insertShopkeeperAudit1 -----------------  开始 Resultmap: "+Resultmap);
		Long addressId = Long.parseLong(String.valueOf(Resultmap
				.get("addressId")));*/
		long end = System.currentTimeMillis();
		System.out.println("耗费了 "+((end - start)/1000)+" 秒");
		//System.out.println("insertShopkeeperAudit1 -----------------  开始 addressId: "+addressId);
		Long id = IdWorker.getIdWorker().nextId();
		map = new HashMap<String, Object>();
		map.put("comp_id", id);// 公司id
		map.put("comp_name", compName);// 公司名称
		...
		try {
			map.put("first_letter", PiyinUtil.getFullNameSpell(compName)
					.toUpperCase().subSequence(0, 1));
			System.out.println("insertShopkeeperAudit1 -----------------  开始 first_letter: "+PiyinUtil.getFullNameSpell(compName)
					.toUpperCase().subSequence(0, 1));
		} catch (Exception e) {
			e.printStackTrace();
		}// 公司名首字母
...
//		map.put("belong_to_net_id", 1501);// 快递加盟公司及站点所属快递网络I -1:代表此字段无意义
		...
		try {
			...
			System.out.println("insertShopkeeperAudit1 -----------------  开始 插入insertBasCompinfoNew: ");
...
//			basCompAuditMapper.insertbasCompAudit(map);
		...
		System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa 店长入驻不存在的代收点 ---- insertShopkeeperAudit1 :"+ id.toString());
		this.redisService.removeAll("findShopowner1xxx");
		this.redisService.removeAll("findShopowner2xxx");
		this.redisService.removeAll("findShopowner3xxx");
		this.redisService.removeAll("findIdentity");
...
//				basCompAuditMapper.insertbasCompAudit(map);
	...
	@Override
	public Map<String, Object> findIdentity(String idNum, String memberPhone) {
		//先验证有没有实名认证，如果有则手机和身份证匹配即可，否则只验证身份证号
		HashMap<String,Object> memMap = collectionMapper.querySfAuditInfo(memberPhone);
		if(PubMethod.isEmpty(memMap)){
			...
			map = this.collectionMapper.findIdentityNew(idNum,memberPhone);
			if(PubMethod.isEmpty(map)){
				map = new HashMap<String, Object>();
				map.put("memberId", 0);
			}
		}
		System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa 验证身份证的::: ---- findIdentity :"+"  身份证::"+idNum+"  ||||  "+ map.get("memberId"));
		return map;
	}
